...
def model_fn():
    ...
    keras_model = create_lstm_cnn_model()
...
keras_model = create_lstm_cnn_model()
# Summary to check the model structure
keras_model.summary()
...
for round_num in range(1, NUM_ROUNDS + 1):
    ...
    current_loss, current_accuracy = evaluate_global_model(keras_model, X_test, y_test)
    # print(f'Round {round_num}, Metrics: {metrics}')

    # early stopping 
    if current_accuracy > best_accuracy + 1e-3:
        best_state = state
...
# keras_model = create_model()
keras_model = create_lstm_cnn_model()
keras_model1 = create_lstm_cnn_model()
# Summary to check the model structure
# keras_model.summary()
...
evaluate_model(keras_model1, X_test, y_test)
